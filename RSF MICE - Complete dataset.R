...
#rfsrc on single imputed datasets by MICE -> datasets created in 'Cox regression - Complete dataset'
RSF_1 <- rfsrc(Surv(time_m, MOF_fracture) ~ Age + Gender + prior_falls + current_vertebral_fracture + pos_family_history + bedridden + backpain + children + COCP + Breastfeeding + duration_menopause +  DM + CVD + IBD + CVA + Epilepsy + Systemic_auto_immune + rheuma_art + malabsorption + renal_insufficiency + corticosteroids +  Collaps_DBC + Delier_dementie_DBC + Duizeligheid_DBC + weigth_under_67 + weight_under_60 + Change_length + Hours_active + Total_daily_calcium + More_6_coffee + Sun_exposure + Fat_fish_diet + veg_diet + Vitam_pills + Daily_margarin + Lab_bse + Lab_calcium + Lab_albumin + Lab_TSH + Lab_vit_d + Lab_GFR_decreased + Lab_GFR_from_90 + T_score_LWK + T_score_HIP + Weight + Length,
...
for (i in 1:30){ 
  ...
  RSF_function <- rfsrc(Surv(time_m, MOF_fracture) ~ Age + Gender + prior_falls + current_vertebral_fracture + pos_family_history + bedridden + backpain + children + COCP + Breastfeeding + duration_menopause +  DM + CVD + IBD + CVA + Epilepsy + Systemic_auto_immune + rheuma_art + malabsorption + renal_insufficiency + corticosteroids + Collaps_DBC + Delier_dementie_DBC + Duizeligheid_DBC + weigth_under_67 + weight_under_60 + Change_length + Hours_active + Total_daily_calcium + More_6_coffee + Sun_exposure + Fat_fish_diet + veg_diet + Vitam_pills + Daily_margarin + Lab_bse + Lab_calcium + Lab_albumin + Lab_TSH + Lab_vit_d + Lab_GFR_decreased + Lab_GFR_from_90 + T_score_LWK + T_score_HIP + Weight + Length,
...
# Variable importance -> from single dataset
