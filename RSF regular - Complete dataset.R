...
length(MOF)
MOF_impute_RF <- MOF[c(3,5:52)]
View(MOF_impute_RF)

#rfsrc with imputation by random forest
RSF_NA <- rfsrc(Surv(time_w, MOF_fracture) ~ Age + Gender + prior_falls + current_vertebral_fracture + pos_family_history + bedridden + backpain + children + COCP + Breastfeeding + duration_menopause +  DM + CVD + IBD + CVA + Epilepsy + Systemic_auto_immune + rheuma_art + malabsorption +  renal_insufficiency + corticosteroids  + Collaps_DBC + Delier_dementie_DBC + Duizeligheid_DBC +  weigth_under_67 + weight_under_60 + Change_length + Hours_active + Total_daily_calcium + More_6_coffee + Sun_exposure + Fat_fish_diet + veg_diet + Vitam_pills + Daily_margarin + Lab_bse + Lab_calcium + Lab_albumin + Lab_TSH + Lab_vit_d + Lab_GFR_decreased + Lab_GFR_from_90 + T_score_LWK + T_score_HIP + Weight + Length,
               data=MOF_impute_RF, ntree=500, splitrule = "bs.gradient", block.size = 10, na.action = "na.impute", nimpute = 30, importance = TRUE)
RSF_NA
...
mean(cv_RSF_NA$BootstrapCrossValCindexMat)
cv_RSF_NA
